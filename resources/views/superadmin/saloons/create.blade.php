...
@section('title', 'Add New Saloon')
@section('page-title', 'Create Saloon')
@section('page-subtitle', 'Register a new saloon into the system')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-lg p-8">
        <form action="{{ route('admin.saloons.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label class="form-label" for="owner_id">Select Owner</label>
                        <select name="owner_id" id="owner_id" class="form-control" required>
                            <option value="">-- Select a Saloon Admin --</option>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Only Saloon Admins without an existing saloon are listed.</p>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="time" name="opening_time" id="opening_time" class="form-control" value="{{ old('opening_time') }}" required>
                    ...
                    <div>
                        ...
                        <input type="time" name="closing_time" id="closing_time" class="form-control" value="{{ old('closing_time') }}" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="subscription_level" id="subscription_level" class="form-control">
                            <option value="silver" {{ old('subscription_level') == 'silver' ? 'selected' : '' }}>Silver (Basic)</option>
                            <option value="gold" {{ old('subscription_level') == 'gold' ? 'selected' : '' }}>Gold (Standard)</option>
                            <option value="platinum" {{ old('subscription_level') == 'platinum' ? 'selected' : '' }}>Platinum (Premium)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="subscription_expires_at" id="subscription_expires_at" class="form-control" value="{{ old('subscription_expires_at') }}">
                        <p class="text-xs text-gray-500 mt-1">Leave blank for indefinite/life-time (if applicable) or manual recharge.</p>
            ...
            <div class="flex items-center justify-end gap-4 mt-8 pt-4 border-t">
                ...
                <button type="submit" class="btn-primary">
                    <i class="fas fa-save mr-2"></i>Create Saloon
                </button>
            </div>
        </form>
    </div>
